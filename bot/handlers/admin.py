...
async def is_admin(message: Message, db) -> bool:
    ...
    result = await db.execute(
        select(Manager).where(Manager.tg_user_id == message.from_user.id)
    )
    mgr = result.scalar_one_or_none()

    if mgr and mgr.role in (Role.admin, Role.superadmin, Role.lead):
        return True
...
async def admin_active_orders(m: Message, db):
    ...
    for o in orders:
        amount_str = f"{float(o.amount):.2f}" if o.amount is not None else "0.00"
        currency = o.currency or ""
        lines.append(
            f"{o.order_number or o.id}: клиент #{o.client_id}, менеджер #{o.manager_id}, "
            f"направление: {o.direction.value}, сумма: {amount_str} {currency}"
        )
